    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto custom-scrollbar">
        ...
        {logs.map(log => {
          ...
          return (
            <div key={log.id} className="p-4 text-xs flex items-start gap-3 hover:bg-slate-50">
              {log.action === 'verify_landlord' && (
                <div className="p-1.5 bg-emerald-100 text-emerald-800 rounded shrink-0">
                  <Shield size={16} />
                </div>
              )}
              {log.action === 'revoke_landlord' && (
                <div className="p-1.5 bg-red-100 text-red-800 rounded shrink-0">
                  <AlertTriangle size={16} />
                </div>
              )}
              {log.action === 'resolve_report' && (
                <div className="p-1.5 bg-blue-100 text-blue-800 rounded shrink-0">
              ...
              <div className="flex-1 space-y-0.5">
                <p className="text-slate-800 font-medium leading-relaxed">
                  ...
                  {log.action === 'verify_landlord' && ` verified agent `}
                  {log.action === 'revoke_landlord' && ` revoked agent `}
                  {log.action === 'resolve_report' && ` resolved report `}
                  <strong className="text-slate-900">{log.target_name}</strong>
                </p>
                {log.reason && (
                  <p className="text-slate-500 italic bg-slate-100 p-2 rounded mt-1">

...
// WhatsApp brand glyph (Material Symbols has no brand icons).
const WhatsAppGlyph: React.FC<{ size?: number }> = ({ size = 14 }) => (
...
export const PropertyDetailPage: React.FC = () => {
  ...
  if (!property) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-panel-strong h-96 rounded-2xl animate-pulse border border-line" />
      </div>
    );
  }
  ...
  return (
    <main className="max-w-6xl mx-auto px-5 sm:px-8 lg:px-10 py-8">
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-fg/50 mb-6">
      ...
      {/* Hero image + scrim + title */}
      <div className="relative rounded-2xl overflow-hidden aspect-[21/9] bg-panel-strong shadow-lift">
        {property.image_url ? (
          <img
          ...
          <div className="w-full h-full bg-panel-strong flex items-center justify-center">
            <MapPinIcon size={32} className="text-fg/20" />
          </div>
        )}
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent pt-24 p-6 sm:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
        ...
        <div className="space-y-8 lg:col-span-2">
          ...
          <section>
            <h2 className="display font-bold text-2xl text-fg mb-4">Units &amp; pricing</h2>
            {categories.length === 0 ? (
              <p className="text-sm text-fg/60 bg-panel rounded-2xl border border-line p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {categories.map((cat, i) => (
                  <Reveal key={cat.id} delay={(i % 2) * 120}>
                    <div className="bg-panel rounded-2xl border border-line p-5 shadow-soft hover:shadow-lift transition-all hover:-translate-y-0.5 flex flex-col h-full">
                        ...
                        <button
                          ...
                          className={`shrink-0 inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold transition ${
                            cat.photos?.length > 0 || cat.video_url
                              ...
                              : 'border border-dashed border-primary/40 bg-primary-light text-primary hover:bg-primary/10'
                          }`}
                        >
                          <PhotoLibraryIcon size={14} />
                          Pics &amp; Videos
                        </button>
                      </div>
                      {cat.description && (
                      ...
                      <div className="mt-auto pt-3 border-t border-line flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="p-3 rounded-full bg-primary-light text-primary hover:bg-primary/20 transition"
                          >
                            <PhoneIcon size={18} />
                          </button>
                          <button
                            ...
                            className="p-3 rounded-full bg-panel-strong text-fg/70 hover:bg-line/60 hover:text-fg transition"
                          >
                            <MessageIcon size={18} />
                          </button>
                          <button
                            ...
                            className="p-3 rounded-full bg-primary text-white hover:bg-primary-dark transition"
                          >
        ...
        <aside className="lg:col-span-1">
          <div className="lg:sticky lg:top-24 space-y-4">
            <div className="bg-panel border border-line rounded-2xl p-6 shadow-soft">
              <h2 className="display font-bold text-xl text-fg mb-2">Ready to view?</h2>
              <p className="text-sm text-fg/60 leading-relaxed mb-5">
                Contact details are revealed only for vacant, verified units — direct from
                the ID-verified manager, never a broker.
              </p>
              <div className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-primary bg-primary-light border border-primary/20 rounded-lg px-2.5 py-1.5">
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 text-xs font-medium text-fg/50 hover:text-red-600 border border-line hover:border-red-300 rounded-lg px-4 py-2.5 transition"
            >
              <FlagIcon size={14} />
